...
<script>
	...
	export default{
		...
		methods:{
            ...
            doRegist () {
            	...
            	if( !account || !nickname || !password ){         			
            		...
            		.then(function (response) {
	            		...
	            		self.dealResponse(response.data)
	            	})
            ...
            doLogin () {
            	var self = this
            	var url  = '/api/user/login?account=' + self.login.account + '&password=' + self.login.password
            	self.$http.get(url)
            	.then(function (response) {
            		self.dealResponse(response.data)
            	})
            ...
            dealResponse (response) {
                ...
                if (response.status) {
					self.$store.state.userName = response.username
